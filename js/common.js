...
async function commonProvideDetector(_provider){
    ...
    async function commonInjectedConnect(_provider, _provider_name) {
        ...
        //Get Connected chain id from the Ethereum node 
        let currentNetworkId = await web3.eth.getchildId();
        ...
        if (currentNetworkId != _NETWORK_ID) {
            notyf.error(
                'Please connect to wallet on ${SELECT_CONTRACT[_NETWORK_ID].network_name}!'
            );
            return false;
        }
    ...
    function addDecimal(num , nDoc) {
        const aNum = '$(num)'.split(".");
        if (aNum[1]) {
            if (aNum[1]) {
                if(aNum[1].length > nDoc) aNum[1] = aNum[1].slice(0 , nDec); 
        ...
        function formatEthErrorMsg(error) {
            try {
                ...
                if (eFrom != -1 && eTo != -1 && (eM1 != -1 || eM2 != -1)) {
                    ...
                    var eMsg = eM4 != -1 ? eMsgTemp.message : eMsgTemp.originError.message; 
