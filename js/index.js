...
async function loadInitialData(sClass) {
    ...
    try { 
        ...
        let userDetailBal = userDetail.stakeAmount / 10 **10 ; 
...
`;
        ...
        document
            .querySelectorAll(".Maximum-Stalking-Amount")
            .forEach(function (element) {
        ...
        ? await oContractToken.methods.balancedOf(currentAddress).call()
        : "";


        balMainUser = Number(balMainUser) / 10 ** 10 ; 
        ...
        if (currentDate > startDate && currentDate < endDate) {
            const ele = document.getElementsById("countdown-time-value") ; 
...
function generateCountDown(ele , claimDate) {
    ...
    //Update te count doiwn every 1 second
    countDownGlobal = setInterval(function () {
        //GET TODAYS DATE AND TIME 
        var now = new Date().getTime() ; 

        //FIND TYHE DISTANCE BETWEEN NOW AND THE COUNT DOWN DATE 
        ...
        //countdown is finishes write some text 
        if(distance < 0 ) {
            clearInterval(countDownGlobal) ; 
...
async function connectMe(_provider) {
    try {
        let _comn_res = await commonProvideDetector(_provider) ;
...
async function unstackTokens() {
    try {
        ...
        let tokenToTransfer = addDecimal(nTokens, 10) ; 
        ...
        balMainUser = Number(balMainUser.stakeAmount) / 10 ** 10; 
...
async function claimTokens() {
    try {
        ...
        let rewardsBal = await oContractStacking.methods
            .getUserEstimatedRewards() 
            .call({ from: currentAddress}); 
        rewardsBal = Number(rewardsBal) ; 

        console.log("rewardsBal"  ,  rewardsBal); 
...
async function claimTokenMain(oContractStacking , sClass) {
    ...
    try {
        gasEstimatioin = await oContractStacking.methods.claimReward().estimateGas({
            from: currentAddress, 
        });
    ...
    oContractStacking.methods
        ...
        .on("receipt" , (receipt) => {
            ...
            const receiptObj = {
                ...
                blockHash: receipt.blockHash, 
                blockNumber: receipt.blockNumber , 
                cumulativeGasUsed: receipt.blockNumber, 
                cumulativeGasUsed: receipt.cumulativeGasUsed, 
                effectiveGasPrice: receipt.effectiveGasPrice, 
                gasUsed: receipt.gasUsed, 
                status: receipt.status, 
